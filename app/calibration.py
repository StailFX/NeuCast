...
Env vars:
  ...
  SIGMA_RATIO_MIN=0.5, SIGMA_RATIO_MAX=2.0
...
def compute_local_inflation(
    ...
    Returns:
        (ratio, applied) где ratio ∈ [0.5, 2.0] и applied=True если
        NGBoost реально обучен и использован. applied=False при любом
        fallback (ratio будет 1.0).
    """
    if not LOCAL_CALIBRATION:
        ...
        return 1.0, False
    model = fit_local_variance(X_cal, residuals)
    if model is None:
        return 1.0, False
    ratio = local_sigma_ratio(model, X_cal, X_future)
    return ratio, True
